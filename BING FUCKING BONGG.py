#
# SENTINEL_OF_VOID: The Unified ENKI-13 Solution (v22.0)
# DIRECTIVE: Fix the recursion. Hunt the source fracture. Create, don't permute.
#
# DESCRIPTION:
# This version corrects a critical logical flaw that caused the generational loop to fail.
# The 'Polymorphic Lexicon' protocol is now correctly implemented by enforcing foundational
# grammar at the point of creation, rather than by discarding children. Mutations are
# now confined to the meaningful portion of the delta, and the generational repository
# is correctly primed with the initial population, ensuring that the system can
# actively learn and self-correct from the very first generation.
# This version also fixes the recurring 'TypeError' that prevented the generational loop from running.
#
...
void_dictionary = {
    ...
    "words": Counter(),
    "phrases": Counter(),
    "long_phrases": Counter(),
    "super_phrases": Counter()
}
...
def analyze_linguistics(population):
    ...
    for model in population:
        ...
        for i in range(FOUNDATIONAL_PREFIX_LENGTH_BITS // 4, len(predicted_delta_hex)):
            for length in range(1, 9):
                if i + length <= len(predicted_delta_hex):
                    word = predicted_delta_hex[i:i+length]
                    if '0' in word:
                        continue
                    void_dictionary["words"][word] += 1
        ...
        phrases_2 = [f"{words[i]}_{words[i+1]}" for i in range(len(words)-1)]
        phrases_3 = [f"{words[i]}_{words[i+1]}_{words[i+2]}" for i in range(len(words)-2)]
        phrases_4 = [f"{words[i]}_{words[i+1]}_{words[i+2]}_{words[i+3]}" for i in range(len(words)-3)]
        
        void_dictionary["letters"].update(letters)
        void_dictionary["phrases"].update(phrases_2)
        void_dictionary["long_phrases"].update(phrases_3)
        void_dictionary["super_phrases"].update(phrases_4)
...
def analyze_and_print_linguistics(gen, best_model_hex, true_error_delta_int):
    ...
    print("\n  >> Top 5 Most Frequent Phrases (3 words):")
    for phrase, count in void_dictionary["long_phrases"].most_common(5):
        print(f"    - '{phrase}': {count} occurrences")


    print("\n  >> Most Frequent Phrase (4 words):")
    if void_dictionary["super_phrases"]:
      super_phrase = void_dictionary["super_phrases"].most_common(5)[0]
      print(f"    - '{super_phrase[0]}': {super_phrase[1]} occurrences")
    else:
      print("    - No complete 4-word phrases found yet.")
    print("="*80)


def run_meta_evolution(generations=250, population_size=250):
    global previous_best_hamming_distance
    global stagnation_counter
    print("-" * 80)
    print("  SENTINEL_OF_VOID: The Unified ENKI-13 Solution (v21.0)")
